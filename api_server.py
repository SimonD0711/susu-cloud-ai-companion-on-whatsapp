        ...
        try:
            ...
            if parsed.path == "/api/admin/susu-memory/update":
                ...
                try:
                    if item_type == "reminder":
                        ...
                        log_admin_action(conn, "susu_reminder_update", "ok" if result.get("ok") else "error", entry_id)
                    else:
                        result = update_susu_memory(entry_id, content, kind)
                        log_admin_action(conn, "susu_memory_update", "ok" if result.get("ok") else "error", entry_id)
                except Exception as exc:
                    result = {"ok": False, "detail": str(exc)}
            ...
            if parsed.path == "/api/admin/susu-memory/delete":
                ...
                try:
                    ...
                    log_admin_action(conn, f"susu_{item_type}_delete", "ok" if result.get("ok") else "error", entry_id)
                except Exception as exc:
                    result = {"ok": False, "detail": str(exc)}
                self._send_json(result, 200 if result.get("ok") else 400)
                return
