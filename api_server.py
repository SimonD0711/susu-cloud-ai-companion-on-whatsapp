...
ADMIN_SESSION_COOKIE = os.environ.get("SUSU_ADMIN_SESSION_COOKIE", "cc_admin_session").strip() or "cc_admin_session"
ADMIN_SESSION_TTL = int(os.environ.get("SUSU_ADMIN_SESSION_TTL", str(60 * 60 * 24 * 30)))
MONGO_URI = os.environ.get("SUSU_MONGO_URI", "").strip()
SITE_MONITORS = json.loads(os.environ.get("SUSU_SITE_MONITORS", "[]")) or []
...
def fetch_site_status():
    ...
    for item in SITE_MONITORS:
        ...
        error_message = ""
        try:
            request = Request(item["url"], method="HEAD", headers={"User-Agent": "SusuCloud-Admin/1.0"})
...
def fetch_service_health():
    ...
    return {
        "services": [
            ...
            read_service_detail(os.environ.get("SUSU_API_SERVICE_NAME", "susu-api")),
        ],
...
const MONGO_URI = process.env.SUSU_MONGO_URI || '';
...
MONGO_URI_LOCAL = os.environ.get("SUSU_MONGO_URI_LOCAL", "").strip()


def run_admin_action(action):
    cache_dir = Path(os.environ.get("SUSU_CACHE_DIR", "/var/cache/nginx"))
    backup_dir = Path(os.environ.get("SUSU_BACKUP_DIR", "/root/backups"))
    ...
    if action == "restart_api":
        subprocess.Popen(
            ["/bin/sh", "-lc", f"sleep 1 && systemctl restart {os.environ.get('SUSU_API_SERVICE_NAME', 'susu-api')} >/tmp/susu-api-restart.log 2>&1"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        return {"ok": True, "detail": "Scheduled API service restart."}
...
def fetch_server_status():
    ...
    cert_configs_raw = os.environ.get("SUSU_CERT_CONFIGS", "")
    certificates = []
    if cert_configs_raw:
        try:
            cert_configs = json.loads(cert_configs_raw)
            for cfg in cert_configs:
                certificates.append(describe_cert(cfg["path"], cfg.get("label", ""), cfg.get("host", "")))
        except Exception:
            pass
    ...
    try:
        return {
            ...
            "services": [
                ...
                {"name": os.environ.get("SUSU_API_SERVICE_NAME", "susu-api"), "status": service_status(os.environ.get("SUSU_API_SERVICE_NAME", "susu-api"))},
            ],
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path.startswith("/api/map-tile/"):
            ...
            try:
                request = Request(tile_url, headers={"User-Agent": "SusuCloud-MapProxy/1.0"})
                with urlopen(request, timeout=8) as response:
                    body = response.read()
