...
BRIDGE_HOST = os.environ.get("WA_BRAIN_BRIDGE_HOST", "127.0.0.1").strip() or "127.0.0.1"
BRIDGE_PORT = int(os.environ.get("WA_BRAIN_BRIDGE_PORT", "9102"))
BRIDGE_API_KEY = os.environ.get("WA_BRAIN_BRIDGE_API_KEY", "").strip()
UPSTREAM_MODE = os.environ.get("WA_BRAIN_BRIDGE_UPSTREAM_MODE", "openai").strip().lower() or "openai"
UPSTREAM_URL = os.environ.get("WA_BRAIN_BRIDGE_UPSTREAM_URL", "").strip()
UPSTREAM_API_KEY = os.environ.get("WA_BRAIN_BRIDGE_UPSTREAM_API_KEY", "").strip()
UPSTREAM_MODEL = os.environ.get("WA_BRAIN_BRIDGE_UPSTREAM_MODEL", "").strip()
UPSTREAM_TIMEOUT_SECONDS = float(os.environ.get("WA_BRAIN_BRIDGE_TIMEOUT", "90"))
UPSTREAM_AUTH_HEADER = os.environ.get("WA_BRAIN_BRIDGE_UPSTREAM_AUTH_HEADER", "Authorization").strip() or "Authorization"
...
def dispatch_upstream(request_payload):
    ...
    if mode in {"openai", "chatbridge", "bridge", ""}:
...
class BridgeHandler(BaseHTTPRequestHandler):
    server_version = "SusuBrainBridge/0.3"
